...
export default {
  ...
  mounted() {
    ...
    const hiddenInput = this.el.querySelector("#editor_hidden_input");
    ...
    if (!hiddenInput.value && this.el.dataset.suggestion) {
      hiddenInput.value = this.el.dataset.suggestion;
    }
          ...
          this.editor.action((ctx) => {
            ...
            listener.updated((ctx, doc, prevDoc) => {
              ...
              this._updateTimeout = setTimeout(() => {
                ...
                // Strip zero-width spaces used to preserve trailing whitespace (see mention insertion)
                markdownContent = markdownContent.replace(/\u200B/g, '');
  ...
  updated() {
    const hiddenInput = this.el.querySelector("#editor_hidden_input");
    if (!this.editor || !hiddenInput) return;
    ...
    this.editor.action((ctx) => {
      const view = ctx.get(editorViewCtx);
      const currentMarkdown = ctx.get(serializerCtx)(view.state.doc);
