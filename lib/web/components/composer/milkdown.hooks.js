...
// State management for mention dropdown control
const mentionDropdownState = {
  skipNext: false
};
...
function mentionsPluginView(view) {
  ...
  const provider = new SlashProvider({
    ...
    shouldShow: (view, prevState) => {
      ...
      if (mentionDropdownState.skipNext) {
        mentionDropdownState.skipNext = false;
        return false;
      }
...
const initEmojiPicker = (editor) => {
  ...
  // Skip if picker already initialized
  if (currentPicker) {
  ...
  try {
    ...
    // The picker loads emojis asynchronously from IndexedDB, which can change the container size
    currentPicker.addEventListener('emoji-ready', () => {
      // Call the tooltip's updatePosition function directly (stored on the element by tooltip.js)
...
const setupLazyEmojiPicker = (editor) => {
  // Find the emoji button in the composer
  ...
  // Initialize picker on first button click
    ...
    if (!hiddenInput || !container) {
      ...
      this.handleEvent("mention_suggestions", (payload) => {
        ...
        if (existingContent && isUrl) {
          ...
          mentionDropdownState.skipNext = true;
        ...
        this.editor = await Editor.make()
            .config((ctx) => {
              ...
              ctx.update(editorViewOptionsCtx, (prev) => ({
                ...prev,
                attributes: { 
                  class: 'milkdown-editor mx-auto focus:outline-hidden h-full p-2 prose text-[16px] prose-sm prose-bonfire break-normal max-w-none text-base-content prose-hr:!my-2 prose-br:hidden', 
                  spellcheck: 'false',
          ...
          setupLazyEmojiPicker(this.editor);
