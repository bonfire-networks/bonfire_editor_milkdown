    ...
    if (!hiddenInput || !container) {
      ...
      this.handleEvent("mention_suggestions", (payload) => {
        ...
        if (text) {
          ...
          const formattedText = formatMention(text);
          ...
          this.editor.action((ctx) => {
            ...
            listener.updated((ctx, doc, prevDoc) => {
              ...
              this._updateTimeout = setTimeout(() => {
                // Get plain text content with line breaks between paragraphs
                const plainTextContent = doc.textBetween(0, doc.content.size, "\n").replace(/\u00A0/g, " ");
