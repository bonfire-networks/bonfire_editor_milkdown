...
export default {
  mounted() {
    ...
    // Defer loading the heavy editor bundle until the composer is actually shown.
    // The composer is present-but-translated-off-screen when closed, so an
    // IntersectionObserver fires when it opens (and immediately if it's already
    // visible, e.g. inline/embedded). The focusin/pointerdown listeners are a
    // fallback in case intersection doesn't fire but the user reaches the editor.
  ...
  // Loads the editor bundle + initializes the editor (at most once), then flushes
  // any server events that were queued while it loaded.
  async loadEditor() {
