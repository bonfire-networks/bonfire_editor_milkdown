...
// Helper function to validate and format mentions for ActivityPub
function formatMentionForActivityPub(mention) {
  // Ensure mention starts with @
  if (!mention.startsWith('@')) {
    mention = '@' + mention;
  }
  
  // Validate remote mention format
  if (mention.includes('@') && mention.indexOf('@') !== 0) {
    // This is a remote mention like @user@domain.com
    const parts = mention.substring(1).split('@');
    if (parts.length === 2 && parts[0] && parts[1]) {
      // Valid remote mention format
      return mention;
    }
  }
      ...
      return getFeedItems(query, "@").then(items => {
        ...
        buttons.forEach(button => {
          button.addEventListener('click', () => {
            ...
            // Calculate position - text here is the full query including remote part
            const matchLength = text.length + 1; // +1 for @ symbol
            const startPos = from - matchLength;
            
            // Format mention properly using validation function
            const formattedMention = formatMentionForActivityPub(mention);
    ...
    try {
      ...
      this.handleEvent("mention_suggestions", (payload) => {
        ...
        if (text) {
          ...
          // Ensure proper mention formatting for ActivityPub
          const formattedText = formatMentionForActivityPub(text);
